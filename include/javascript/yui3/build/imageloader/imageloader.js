...
if(this.get('bgUrl')!==null){if(this.get('isPng')&&Y.UA.ie&&Y.UA.ie<=6){el.setStyle('filter','progid:DXImageTransform.Microsoft.AlphaImageLoader(src="'+this.get('url')+'", sizingMethod="'+this.get('sizingMethod')+'", enabled="'+this.get('enabled')+'")');}
...
this._fetched=true;return true;},_getImgEl:function(){if(this._imgEl===null){this._imgEl=Y.get('#'+this.get('domId'));}
return this._imgEl;},_getYPos:function(){if(this._yPos===null){this._yPos=this._getImgEl().getY();}
return this._yPos;}};Y.extend(Y.ImgLoadImgObj,Y.Base,imgProto);},'3.0.0',{requires:['base-base','node-style','node-screen']});